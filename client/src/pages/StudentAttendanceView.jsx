...
import { fetchStudentInformation ,fetchStudentAttendance} from '../services/attendanceService.jsx';
...
const fetchAttendance = async () => {
  ...
  const displayData = viewMode === 'date'
    ? groupByDate()
    : (selectedSubject ? { [selectedSubject]: filteredAttendance } : groupedBySubject);

  return (
    <div style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto' }}>
      <h1 style={{ marginBottom: '30px' }}>My Attendance</h1>
      ...
      {studentInfo && (
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
            <div>
              <span style={{ fontWeight: 'bold' }}>Name:</span> {studentInfo.name}
            </div>
            <div>
              <span style={{ fontWeight: 'bold' }}>Email:</span> {studentInfo.email}
            </div>
            <div>
              <span style={{ fontWeight: 'bold' }}>Roll Section:</span> {studentInfo.section || 'N/A'}
            </div>
            <div>
              <span style={{ fontWeight: 'bold' }}>Department:</span> {studentInfo.department.departmentName || 'N/A'}
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#155724', marginTop: '5px' }}>Present</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#721c24', marginTop: '5px' }}>Absent</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#856404', marginTop: '5px' }}>Late</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#0c5460', marginTop: '5px' }}>Excused</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#2e7d32', marginTop: '5px' }}>Total Classes</div>
        </div>

        <div style={{
          backgroundColor: percentage >= 75 ? '#c8e6c9' : percentage >= 60 ? '#fff9c4' : '#ffccbc',
          padding: '15px',
          borderRadius: '8px',
          textAlign: 'center',
          borderLeft: `4px solid ${percentage >= 75 ? '#43a047' : percentage >= 60 ? '#f9a825' : '#e64a19'}`
        }}>
          <div style={{
            fontSize: '24px',
            fontWeight: 'bold',
            color: percentage >= 75 ? '#2e7d32' : percentage >= 60 ? '#f57f17' : '#d84315'
          }}>
            {percentage}%
          </div>
          <div style={{
      ...
      {Object.keys(displayData).length > 0 ? (
        <div>
          <h2 style={{ marginBottom: '20px' }}>
            Attendance Details ({viewMode === 'date' ? 'By Date' : 'By Subject'})
          </h2>
          ...
          {Object.entries(displayData).map(([key, records]) => {
            ...
            const keyPercentage = keyStats.total > 0
              ? ((keyStats.present / keyStats.total) * 100).toFixed(1)
              : 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <div>
                            <span style={{ opacity: 0.7 }}>Teacher:</span>
