          ...
          const activeSlides = data.data
            ...
            .map((slide) => ({
              ...
              imageUrl: slide.imageUrl.startsWith('http')
                ? slide.imageUrl
                : `${API_URL}${slide.imageUrl}`,
            }));

          if (activeSlides.length === 0) throw new Error('No active slides found');
    ...
    <div style={styles.heroContainer}>
      ...
      <style>{`
        ...
        .hero-nav-button {
          ...
          border-radius:50%; font-size:1.5rem; cursor:pointer; z-index:3; display:flex; align-items:center; justify-content:center;
        }
        .hero-nav-button:hover { background: rgba(255,255,255,0.5); }
        .hero-nav-button.prev { left:20px; }
        .hero-nav-button.next { right:20px; }
        .hero-indicators { position:absolute; bottom:20px; left:50%; transform:translateX(-50%); display:flex; gap:10px; z-index:3; }
        .hero-indicator { width:12px; height:12px; border-radius:50%; background: rgba(255,255,255,0.5); border:none; cursor:pointer; }
        .hero-indicator.active { background:#fff; width:30px; border-radius:6px; }
        @media (max-width:768px) { .hero-nav-button { width:40px; height:40px; font-size:1.2rem; } }
      `}</style>
    </div>
  );
};
